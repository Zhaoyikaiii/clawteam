...
```mermaid
flowchart LR
    subgraph Problems["调度要解决的 5 个问题"]
        A[1. 什么时候触发?]
        B[2. 调用哪个 Agent?]
        C[3. 并发多少?]
        D[4. 权限够吗?]
...
```mermaid
flowchart TD
    Start([收到消息]) --> onMessage[onMessage]

    onMessage --> Step1[Step 1: 构建上下文]
    Step1 --> Step2[Step 2: 意图识别]
    Step2 --> Step3[Step 3: 触发判定]

    Step3 --> Explicit{显式 @?}
    Explicit -->|是| AddExplicit[添加 @ 的 Agent]
    Explicit -->|否| Implicit{隐式意图?}

    Implicit -->|置信度够| AddImplicit[添加推荐 Agent]
    Implicit -->|否| NoAgent[无 Agent]

    AddExplicit --> Step4[Step 4: 召回候选]
    AddImplicit --> Step4

    Step4 --> Step5[Step 5: 排序打分]
    Step5 --> Step6[Step 6: 权限检查]

    Step6 --> Policy{权限决策}
    Policy -->|Deny| LogDeny[记录拒绝]
    Policy -->|Require Approval| CreateApproval[创建审批]
    Policy -->|Allow| Step7[Step 7: 调度执行]

    CreateApproval --> WaitUser[等待用户]
    WaitUser --> UserDec{用户决策}
    UserDec -->|批准| Step7
    UserDec -->|拒绝| LogDeny

    Step7 --> RunJobs[运行 Jobs]
    RunJobs --> Step8[Step 8: 结果合并]

    Step8 --> Step9[Step 9: 回写记忆]
    Step9 --> Step10[Step 10: 发送回复]
    Step10 --> End([完成])
...
```mermaid
flowchart TD
    Input[用户请求] --> Complexity{复杂度?}

    Complexity -->|简单| Local[本地模型<br/>轻量快速]
    Complexity -->|中等| CloudSmall[云端小模型<br/>平衡]
    Complexity -->|复杂| CloudLarge[云端大模型<br/>高质量]

    Local --> Output[返回结果]
    CloudSmall --> Output
    CloudLarge --> Output

    CloudSmall -.-> Cache[缓存结果]
    CloudLarge -.-> Cache
```
